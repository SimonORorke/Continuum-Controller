...
local CAT_OTHER1 = 14
local KEY_FIRMWARE_VERSION = "FirmwareVersion"
local KEY_SYSTEM_PRESET_CATEGORIES = "SystemPresetCategories"
  ...
  local msg = midiMessage
    ...
    if (msg.controllerNumber==109 and msg.value==40) then
     ...
     if (msg.controllerNumber==56 and msg.value==0) then
         ...
         if isUpdatingSystemPresets then
...
function onEndOfSystemPresetList()
    ...
    isSystemPresetsUpdateRequired = false
...
function onFirmwareVersionReceived()
    ...
    isSystemPresetsUpdateRequired = 
        not persistableItems 
        or firmwareVersion ~= persistableItems.firmwareVersion
end
...
function restorePersistableData()
    print("restorePersistableData")
    recall(persistableItems)
    if persistableItems then
        local persistedSystemPresetCategories = persistableItems[KEY_SYSTEM_PRESET_CATEGORIES]
        if persistedSystemPresetCategories then
            print("    Restoring systemPresetCategories")
            systemPresetCategories = persistedSystemPresetCategories
            print("    Restored systemPresetCategories")
        end
    end
end
...
function savePersistableData()
    ...
    if haveSystemPresetsBeenUpdated then
        print("    Saving")
        persistableItems = {}
        persistableItems[KEY_FIRMWARE_VERSION] = firmwareVersion
        persistableItems[KEY_SYSTEM_PRESET_CATEGORIES] = systemPresetCategories
        persist(persistableItems)
        print("    Saved")
    end
end
